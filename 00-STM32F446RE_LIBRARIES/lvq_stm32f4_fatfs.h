 ...
 * @ide     Keil uVision
 * @license GNU GPL v3
 * @brief   Fatfs implementation for SLVQ32F4xx devices
 *
@verbatim
   ----------------------------------------------------------------------
...
/**
 * @defgroup LVQ_FATFS
 * @brief    FATFS implementation for SLVQ32F4xx devices - http://stm32f4-discovery.net/2014/07/library-21-read-sd-card-fatfs-stm32f4xx-devices/ - http://stm32f4-discovery.net/2014/08/library-29-usb-msc-host-usb-flash-drive-stm32f4xx-devices
 *
 * @{
 *
 * FatFs implementation for SLVQ32F4xx devices
 *
...
@verbatim
 ...
 /                  |       |NAME       SLVQ32F4XX       DESCRIPTION             |NAME   SLVQ32F4XX   DESCRIPTION
/ 9                 |       |           4-BIT   1-BIT                           |
|                   |       |                                                   |
 ...
 *
@verbatim
//Set your SPI, for corresponding pins look at LVQ SPI library
 ...
 *
 * As of version 1.7, FATFS can now be used with SDRAM memory on SLVQ32F429-Discovery or SLVQ324x9-EVAL board.
 *
 ...
 *
 * There are 2 functions for search, one is @ref LVQ_FATFS_Search which you should call when you want to do a search operations in your FATFS structure
 *
 * Second is @ref LVQ_FATFS_SearchCallback which is called anytime file/folder is found on system so user can do it's job with file.
 *
...
@verbatim
 ...
 - SLVQ32F4xx RCC
 - SLVQ32F4xx GPIO
 - SLVQ32Fx44 SPI    (only when SPI)
 - SLVQ32F4xx DMA    (only when SDIO)
 - SLVQ32Fx44 SDIO   (only when SDIO)
 - MISC             (only when SDIO)
 - defines.h
 - LVQ SPI           (only when SPI)
 - LVQ DELAY         (only when SPI)
 - LVQ GPIO
 - LVQ SDRAM         (only when SDRAM)
 - FatFS by Chan
@endverbatim
 */
 ...
 * @param   *SizeStruct: Pointer to empty @ref LVQ_FATFS_Size_t structure to store data about memory
 * @retval  FRESULT structure members. If data are valid, FR_OK is returned
 * @example Get memory sizes of USB device:
 *             LVQ_FATFS_GetDriveSize("USB:", &SizeStruct);
 */
FRESULT LVQ_FATFS_GetDriveSize(char* str, LVQ_FATFS_Size_t* SizeStruct);

/**
 * @brief  Gets SDCARD drive size and free space
 * @note   Function is deprecated and will be removed, use @ref LVQ_FATFS_GetDriveSize instead!
 ...
 * @retval FRESULT structure members. If data are valid, FR_OK is returned
 */
FRESULT LVQ_FATFS_DriveSize(uint32_t* total, uint32_t* free);
 ...
 * @retval FRESULT structure members. If data are valid, FR_OK is returned
 */
FRESULT LVQ_FATFS_USBDriveSize(uint32_t* total, uint32_t* free);
 ...
 *  - Call @ref LVQ_FATFS_TruncateBeginning(&opened_file, 5);
 ...
 */
FRESULT LVQ_FATFS_TruncateBeginning(FIL* fil, uint32_t index);
 ...
 *            - > 0: Card is inserted
 */
uint8_t LVQ_FATFS_CheckCardDetectPin(void);
...
int user_func(void) {
    ...
    FRESULT res;
    LVQ_FATFS_Search_t FindStructure;

    //mount first

    if ((res = LVQ_FATFS_Search("SD:", working_buffer, sizeof(working_buffer), &FindStructure)) == FR_OK) {
        //Search was OK
...
uint8_t LVQ_FATFS_SearchCallback(char* path, uint8_t is_file, LVQ_FATFS_Search_t* FindStructure) {
 ...
 * @param  *FindStructure: Pointer to @ref LVQ_FATFS_Search_t structure
 * @retval Member of @ref FRESULT enumeration
 */
FRESULT LVQ_FATFS_Search(char* Folder, char* tmp_buffer, uint16_t tmp_buffer_size, LVQ_FATFS_Search_t* FindStructure);
 ...
 *            - 0: Item is folder
 *            - > 0: Item is file
 * @param  Pointer to @ref LVQ_FATFS_Search_t structure which was passed to @ref LVQ_FATFS_Search with updated data
