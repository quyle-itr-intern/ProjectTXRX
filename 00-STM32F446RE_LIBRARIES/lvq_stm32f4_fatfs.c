...
/* Private functions */
static FRESULT scan_files(char* path, uint16_t tmp_buffer_size, LVQ_FATFS_Search_t* FindStructure);

FRESULT
LVQ_FATFS_GetDriveSize(char* str, LVQ_FATFS_Size_t* SizeStruct) {
    FATFS* fs;
    DWORD fre_clust;
    FRESULT res;
...
FRESULT
LVQ_FATFS_DriveSize(uint32_t* total, uint32_t* free) {
    FATFS* fs;
    DWORD fre_clust;
    FRESULT res;
...
FRESULT
LVQ_FATFS_USBDriveSize(uint32_t* total, uint32_t* free) {
    FATFS* fs;
    DWORD fre_clust;
    FRESULT res;
...
FRESULT
LVQ_FATFS_TruncateBeginning(FIL* fil, uint32_t index) {
...
uint8_t
LVQ_FATFS_CheckCardDetectPin(void) {
    uint8_t status = 1;

#if FATFS_USE_DETECT_PIN > 0
    if (LVQ_GPIO_GetInputPinValue(FATFS_USE_DETECT_PIN_PORT, FATFS_USE_DETECT_PIN_PIN) != 0) {
        status = 0;
    }
#endif
...
FRESULT
LVQ_FATFS_Search(char* Folder, char* tmp_buffer, uint16_t tmp_buffer_size, LVQ_FATFS_Search_t* FindStructure) {
...
/*******************************************************************/
__weak uint8_t
LVQ_FATFS_SearchCallback(char* path, uint8_t is_file, LVQ_FATFS_Search_t* FindStructure) {
    /* NOTE: This function Should not be modified, when the callback is needed,
             the LVQ_FATFS_SearchCallback could be implemented in the user file
    */
...
/*******************************************************************/
static FRESULT
scan_files(char* path, uint16_t tmp_buffer_size, LVQ_FATFS_Search_t* FindStructure) {
    FRESULT res;
    FILINFO fno;
    DIR dir;
    int i;
    ...
    if ((res = f_opendir(&dir, path)) == FR_OK) {
        ...
        while ((res = f_readdir(&dir, &fno)) == FR_OK && fno.fname[0] != 0) {
            ...
            if (fno.fattrib & AM_DIR) {
                ...
                /* Call user function */
                gonext = LVQ_FATFS_SearchCallback(path, 0, FindStructure);
                ...
                /* Call user function */
                gonext = LVQ_FATFS_SearchCallback(path, 1, FindStructure);
            }
