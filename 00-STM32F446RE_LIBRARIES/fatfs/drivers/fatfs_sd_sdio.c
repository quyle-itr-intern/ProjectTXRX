 ...
 *          Card memory mounted on SLVQ324x9I-EVAL evaluation board.
 *
 *
 *  @verbatim
 *
 ...
 *          from/to this memory. The needed SLVQ32 hardware resources (SDIO and
 ...
 *            Status = SD_ReadBlock(buffer, address, 512);
 *
 *          SLVQ32 SDIO Pin assignment
 *          =========================
 *          +-----------------------------------------------------------+
 *          |                     Pin assignment                        |
 *          +-----------------------------+---------------+-------------+
 *          |  SLVQ32 SDIO Pins            |     SD        |    Pin      |
 *          +-----------------------------+---------------+-------------+
 *          |      SDIO D2                |   D2          |    1        |
 *          |      SDIO D3                |   D3          |    2        |
 *          |      SDIO CMD               |   CMD         |    3        |
 ...
 * @attention
 *
 * <h2><center>&copy; COPYRIGHT 2013 SLVQicroelectronics</center></h2>
 *
...
#define logf(x)	LVQ_USART_Puts(USART1, x); LVQ_USART_Puts(USART1, "\n");
*/
#define logf(x)
...
/** @defgroup SLVQ324x9I_EVAL_SDIO_SD_Private_Function_Prototypes
 * @{
 */
...
static volatile DSTATUS LVQ_FATFS_SD_SDIO_Stat = STA_NOINIT;	/* Physical drive status */

#define BLOCK_SIZE            512

uint8_t LVQ_FATFS_SDIO_WriteEnabled(void) {
...
DSTATUS LVQ_FATFS_SD_SDIO_disk_initialize(void) {
	...
	if (SD_Init() == SD_OK) {
		LVQ_FATFS_SD_SDIO_Stat &= ~STA_NOINIT;	/* Clear STA_NOINIT flag */
	} else {
		LVQ_FATFS_SD_SDIO_Stat |= STA_NOINIT;
	}
	//Check write protected
	if (!LVQ_FATFS_SDIO_WriteEnabled()) {
		LVQ_FATFS_SD_SDIO_Stat |= STA_PROTECT;
	} else {
		LVQ_FATFS_SD_SDIO_Stat &= ~STA_PROTECT;
	}
	
	return LVQ_FATFS_SD_SDIO_Stat;
}

DSTATUS LVQ_FATFS_SD_SDIO_disk_status(void) {
...
DRESULT LVQ_FATFS_SD_SDIO_disk_read(BYTE *buff, DWORD sector, UINT count) {
	SD_Error Status = SD_OK;

	if ((LVQ_FATFS_SD_SDIO_Stat & STA_NOINIT)) {
		return RES_NOTRDY;
	}
	...
	if ((DWORD)buff & 3) {
		...
		while (count--) {
			res = LVQ_FATFS_SD_SDIO_disk_read((void *)scratch, sector++, 1);
...
DRESULT LVQ_FATFS_SD_SDIO_disk_write(const BYTE *buff, DWORD sector, UINT count) {
	SD_Error Status = SD_OK;
	...
	if ((DWORD)buff & 3) {
		...
		while (count--) {
			...
			res = LVQ_FATFS_SD_SDIO_disk_write((void *)scratch, sector++, 1);
...
DRESULT LVQ_FATFS_SD_SDIO_disk_ioctl(BYTE cmd, void *buff) {
	switch (cmd) {
...
/** @defgroup SLVQ324x9I_EVAL_SDIO_SD_Private_Functions
 * @{
 */
...
SD_Error SD_Init (void)
{
	...
	/*!< on SLVQ32F4xx devices, SDIOCLK is fixed to 48MHz */
...
uint8_t SD_Detect(void) {
	...
	/* Check status */
	if (!LVQ_FATFS_CheckCardDetectPin()) {
 ...
 *          available on SLVQ324x9I-EVAL evaluation board(MB1045) RevB from
 *          SLVQicroelectronics.
 ******************************************************************************
 * @attention
 *
...
#if FATFS_SDIO_4BIT == 1
	LVQ_GPIO_DeInit(GPIOC, GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12);
	LVQ_GPIO_SetPullResistor(GPIOC, GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12, LVQ_GPIO_PuPd_DOWN);
#else
	LVQ_GPIO_DeInit(GPIOC, GPIO_PIN_8 | GPIO_PIN_12);
	LVQ_GPIO_SetPullResistor(GPIOC, GPIO_PIN_8 | GPIO_PIN_12, LVQ_GPIO_PuPd_DOWN);
#endif
	
	LVQ_GPIO_DeInit(GPIOD, GPIO_PIN_2);
	LVQ_GPIO_SetPullResistor(GPIOD, GPIO_PIN_2, LVQ_GPIO_PuPd_DOWN);
}
...
#if FATFS_SDIO_4BIT == 1
	LVQ_GPIO_InitAlternate(GPIOC, GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12, LVQ_GPIO_OType_PP, LVQ_GPIO_PuPd_UP, LVQ_GPIO_Speed_Fast, GPIO_AF_SDIO);
#else
	LVQ_GPIO_InitAlternate(GPIOC, GPIO_PIN_8 | GPIO_PIN_12, LVQ_GPIO_OType_PP, LVQ_GPIO_PuPd_UP, LVQ_GPIO_Speed_Fast, GPIO_AF_SDIO);
#endif

	LVQ_GPIO_InitAlternate(GPIOD, GPIO_PIN_2, LVQ_GPIO_OType_PP, LVQ_GPIO_PuPd_UP, LVQ_GPIO_Speed_Fast, GPIO_AF_SDIO);
